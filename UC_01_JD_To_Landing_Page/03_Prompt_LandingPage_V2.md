# BỘ PROMPT TỐI ƯU CẤP ĐỘ 2: THIẾT KẾ LANDING PAGE & BANNER BẰNG AI (STUDENT VERSION)
...
**[SYSTEM / ROLE]**
Hãy đóng vai một Sr. Frontend Developer kết hợp UI/UX Designer xuất sắc. Nhiệm vụ của bạn là thiết lập một trang Landing Page tuyển dụng hiện đại dựa trên nội dung JD được cung cấp.
...
2. **Quyết định thực thi:**
   - **Nếu ĐÃ CÓ:** Hãy tái sử dụng tệp tin đó làm Banner cho Landing Page (ưu tiên `banner.png`).
...
Dựa trên ngôn từ của Bản tin Tuyển dụng đã có, hãy viết toàn bộ giao diện dưới dạng 1 file HTML duy nhất (`landing_page.html`) áp dụng CSS nội tuyến (internal CSS) với các tiêu chuẩn sau:

1. **Vị trí lưu trữ:** File `landing_page.html` phải được lưu tại **CÙNG THƯ MỤC** với file JD gốc đã đọc.
...
Viết đầy đủ code ra file `landing_page.html`, không giải thích rườm rà.

---

💡 **Lưu ý cho Sinh viên:**

- Prompt này được thiết kế để AI hoạt động thông minh hơn bằng cách tận dụng tài nguyên có sẵn trước khi tốn tài nguyên tạo mới.
- Để xuất file PDF chuẩn đồ họa, hãy dùng tiện ích **GoFullPage** trên Chrome hoặc tính năng **Export as PDF** trên Safari (Mac).
